...
variable "nat_ip_configurations" {
  type = list(object({
    ...
    subnet_id                     = string
    primary                       = bool
    private_ip_address            = optional(string, "10.0.3.4")
  ...
  validation {
    ...
    error_message = "You can define a maximum of 8 NAT IP configurations."
  }
...
variable "auto_approval" {
  description = "Configuration for automatic approval of private endpoint connections."
  type = object({
...
variable "visibility" {
  description = "Configuration for which subscriptions can view this Private Link Service."
  type = object({
    subscription_ids = optional(list(string), [])
  })
  default = null
